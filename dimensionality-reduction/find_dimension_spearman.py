'''
This script is used to find the optimal SVD dimension for the subset of SimLex-999 with 50, 100, 200 and 500 pairs. We will use full SVD (Singular Value Decomposition) to find the optimal dimension.
Instructions:
1. Change the subset path to the desired subset size.
2. In the plots/ path, append the subset size to the file name at the end. (Eg: svd_dimension_analysis_500.png, svd_dimension_results_table_500.png)
3. Do the same for the pickle file.
4. Run the script.
'''
...
def test_svd_dimensions(cooc_matrix, word2idx, simlex_pairs, dimensions=[50, 100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 950, 1000, 1050, 1100, 1150, 1200, 1250, 1300, 1350, 1400, 1450, 1500]):
    results = {}
    ...
    plt.tight_layout()
    plt.savefig('plots/svd_dimension_analysis_500.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig('plots/svd_dimension_results_table_500.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    simlex_pairs = []
    try:
        with open('data/simlex_subset_500.txt', 'r') as f:
            for line in f:
    ...
    dimensions = [50, 100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 950, 1000, 1050, 1100, 1150, 1200, 1250, 1300, 1350, 1400, 1450, 1500]
    results, best_dim = test_svd_dimensions(cooc_matrix, word2idx, simlex_pairs, dimensions)
    
    with open('svd_analysis_results_500.pkl', 'wb') as f:
        pickle.dump({
            'results': results,
