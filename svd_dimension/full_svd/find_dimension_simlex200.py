'''
This script is used to find the optimal SVD dimension for a subset of SimLex-999 with 200 pairs. We will use full SVD (Singular Value Decomposition) to find the optimal dimension.
'''
...
def test_svd_dimensions(cooc_matrix, word2idx, simlex_pairs, dimensions=[50, 100, 150, 200, 250, 300, 350, 400, 450, 500]):
    ...
    plt.tight_layout()
    plt.savefig('plots/svd_dimension_analysis_200.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig('plots/svd_dimension_results_table_200.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    simlex_pairs = []
    try:
        with open('data/simlex_subset_200.txt', 'r') as f:
            for line in f:
    ...
    with open('svd_analysis_results_200.pkl', 'wb') as f:
        pickle.dump({
            'results': results,
