...
def create_cooccurrence_matrix(file_path, word2idx, window_size):
    ...
    if abs(symmetry_diff) > 1e-10:
        ...
        print("Verified: Matrix is symmetric.")
        
    return cooc_matrix

def apply_ppmi(matrix):
    """Apply Positive Pointwise Mutual Information transformation"""
    # Convert to dense for PPMI calculation
    ...
    # Calculate PMI with better numerical stability
    pmi = np.log2(matrix) + np.log2(total) - np.log2(row_sums) - np.log2(col_sums)
    
    # Handle any remaining NaN or inf values
    pmi = np.nan_to_num(pmi, nan=0.0, posinf=0.0, neginf=0.0)
    
    # Apply PPMI (set negative values to 0)
...
def plot_evaluation_results(results):
    ...
    for metric_name, (values, title, color, ylabel) in metrics.items():
        ...
        plt.tight_layout()
        try:
            plt.savefig(f'plots/{metric_name}_analysis_fixed_spearman.png', dpi=300, bbox_inches='tight')
        except Exception as e:
    ...
    try:
        plt.savefig('plots/metrics_comparison_fixed_spearman.png', dpi=300, bbox_inches='tight')
    except Exception as e:
...
def download_simlex_subset():
    ...
    if os.path.exists(simlex_path):
        ...
        return simlex_path
    print(f"WARNING: SimLex subset file not found at {simlex_path}")
    return simlex_path
...
def verify_window_size(cooc_matrices, word2idx, recommended_window):
    ...
    if not os.path.exists(simlex_path):
        print(f"ERROR: SimLex subset file not found at {simlex_path}")
        return False
    ...
    try:
        ...
        print(f"Error reading SimLex file: {str(e)}")
        return False
    ...
    try:
        plt.savefig('plots/spearman_correlation_fixed.png', dpi=300, bbox_inches='tight')
    except Exception as e:
...
def main():
    ...
    if coverage_inflections:
        ...
        print("\nNo inflection points found in coverage analysis.")
        print(f"Using default recommended window size: {recommended_window}")
    
    print("\nVerifying window size with SimLex-999 subset...")
    verify_result = verify_window_size(cooc_matrices, word2idx, recommended_window)
    
    if verify_result:
        print("\nNOTE: The evaluation suggests window size", recommended_window, "performs well!")
    else:
        print("\nNOTE: Consider exploring alternative window sizes based on the evaluation results.")
